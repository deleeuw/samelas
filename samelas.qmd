...
abstract: The multidimensional scaling methods of McGee and Sammon are adapted to the 
  alternating least squares framework of smacof. This entails using the majorization 
  approach to improve configurations and using variations of monotone regression to improve the
  fit of distances to dissimilarities. It also implies that we describe, for the first time,
  a non-metric version of Sammon Mapping. Implementations in R and C of metric and non-metric versions of McGee's Elastic Scaling and of Sammon Mapping are provided.
---
...
```{r loadshlib, echo = FALSE}
dyn.load("smacofSSSamelas.so")
```

```{r loadRfiles, echo = FALSE}
...
<https://github.com/deleeuw/samelas> 

\sectionbreak

# Introduction

In 1962 Roger N. Shepard published an algorithm and computer program for a non-metric Multidimensional Scaling (MDS) technique that iteratively fits distances to given dissimilarities by minimizing a least squares loss function (@shepard_62a, @shepard_62b). See also @deleeuw_E_17e. Shepard's technique is non-metric in the sense that its results are invariant over monotone transformations of the data, and consequently the results are a function of the rank order of the dissimilarities only. There were some heuristic components in Shepard's work, which were subsequently removed in a reformulation of non-metric MDS by Joseph B. Kruskal (@kruskal_64a, @kruskal_64b). Kruskal's MDS became the standard, not only in
MDS, but in many related forms of non-metric data analysis (@meulman_16). The major renovations Kruskal introduced in psychometrics were the use of gradient methods to minimize differentiable loss functions and the use of monotone regression to find an optimal transformation of the dissimilarities.

In @guttman_68 Louis Guttman presented an alternative to Kruskal's non-metric MDS. It also used least squares
loss functions and gradient-type methods of minimization, but it replaced monotone regression by rank 
images. Very much like Kruskal's approach, the Guttman MDS method was built out to a system of non-metric scaling methods by James C. Lingoes, Edward E. C. H. Roskam, and Ingwer Borg. See the collection @lingoes_roskam_borg_79.

Almost simultaneously with the seminal contributions of Shepard/Kruskal and Guttman/Lingoes/Roskam/Borg, there was *Elastic Scaling* proposed by Victor E. McGee in a series of papers (@mcgee_65, @mcgee_66, @mcgee_67, @mcgee_68) and *Sammon Mapping*, proposed by John W. Sammon Jr. (@sammon_69).

The Elastic method has been largely forgotten, but it is worth preserving, because it is different in some important aspects from the more well-known methods based on Kruskal's stress. In engineering and computer science Sammon Mapping is still a popular dimension reduction method. It was originally intended to map points from a higher-dimensional Euclidean space into points in a lower-dimensional Euclidean space by approximating the known higher dimensional distances by best-fitting lower dimensional ones. 
...
\end{equation}
which McGee calls *work*. We shall just call it *stress* (and *elastic stress* or *McGee stress*), using the more familiar MDS name for loss. We will also continue to use the symbol $\sigma$ for any of the least squares MDS loss functions.
...
\begin{equation}
\sigma(X,\Delta)=\frac{1}{\mathop{\sum\sum}_{1\leq i<j\leq n}w_{ij}\delta_{ij}}\mathop{\sum\sum}_{1\leq i<j\leq n}w_{ij}\frac{(\delta_{ij}-d_{ij}(X))^2}{\delta_{ij}}.\label{eq-sammon}
\end{equation}

The metric MDS problem we address in this paper is minimizing \eqref{eq-sammon} over
the $n\times p$ *configurations* $X$. In the non-metric case we minimize in addition over the
symmetric non-negative and hollow matrices $\Delta:=\{\delta_{ij}\}$, where the $\delta_{ij}$ are monotone with the given dissimilarities. 


The *MASS* package for R implements the *sammon()*
...
If we compare \eqref{eq-mcgee} and \eqref{eq-kruskal} we see that in Kruskal's stress
...
us from considering the trivial zero-residual solution with $X=0$ and $\Delta=0$. In Elastic
